...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        configs.saved_fn = 'fpn_resnet' # 'The name using for saving logs, models,...')
        ...
        configs.batch_size = 1 # 'mini-batch size (default: 4)')
        configs.num_samples = None # type=int, default=None, # 'Take a subset of the dataset to run and debug'
        configs.num_workers = 1 # 'Number of threads for loading data')
        configs.peak_thresh = 0.2
        configs.save_test_output = False # 'If true, the output image of the testing phase will be saved')
...
def detect_objects(input_bev_maps, model, configs):
    ...
    if any_detection:
        print("Detections has objects detected.")
