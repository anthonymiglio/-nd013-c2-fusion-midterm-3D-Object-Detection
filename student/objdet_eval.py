...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            label_bbox = Polygon(tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width, label.box.length, label.box.heading))

            ## step 2 : loop over all detected objects
            for detection in detections:
                ## step 3 : extract the four corners of the current detection
                class_id, x, y, z, h, w, l, yaw = detection
                det_bbox = Polygon(tools.compute_box_corners(x, y, w, l, yaw))
                ...
                iou = label_bbox.intersection(det_bbox).area / label_bbox.union(det_bbox).area
...
def compute_performance_stats(det_performance_all):
    ...
    all_positives, true_positives, false_negatives, false_positives = np.sum(np.array(pos_negs), axis=0)
