...
exercise = 'ID_S1_EX1' # 'ID_S1_EX1', 'ID_S1_EX2', 'ID_S2_EX1', 'ID_S2_EX2', 'ID_S2_EX3', 
                       # 'ID_S3_EX1', 'ID_S3_EX2', 'ID_S4_EX1', 'ID_S4_EX2', 'ID_S4_EX3'
...
set_param = {
    ...
    'ID_S3_EX1':{
        ...
        'files':'/student/objdet_detect.py'},
    'ID_S3_EX2':{
        'sequence':1,
        'show_only_frames':[50, 51],
        'model':"fpn_resnet",
...
sequence = set_param[exercise]['sequence'] # int from 1 to 3
data_filename = ['training_segment-1005081002024129653_5313_150_5333_150_with_camera_labels.tfrecord',
                 'training_segment-10072231702153043603_5725_000_5745_000_with_camera_labels.tfrecord',
                 'training_segment-10963653239323173269_1924_000_1944_000_with_camera_labels.tfrecord'][sequence - 1]

show_only_frames = set_param[exercise]['show_only_frames'] # show only frames in interval for debugging

## Prepare Waymo Open Dataset file for loading
data_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'dataset', data_filename) # adjustable path in case this script is called from another working directory

model = set_param[exercise]['model'] # 'darknet', 'fpn_resnet'

sequence = str(sequence) # "1"

aux = 'resnet' if model == 'fpn_resnet' else model
results_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results/' + model + '/results_sequence_' + sequence + '_' + aux)
...
exec_detection = set_param[exercise]['exec_detection'] # options are 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'; options not in the list will be loaded from file
exec_tracking = set_param[exercise]['exec_tracking'] # options are 'perform_tracking'
exec_visualization = set_param[exercise]['exec_visualization'] # options are 'show_range_image', 'show_bev', 'show_pcl', 'show_labels_in_image', 'show_objects_and_labels_in_bev', 'show_objects_in_bev_labels_in_camera', 'show_tracks', 'show_detection_performance', 'make_tracking_movie'
